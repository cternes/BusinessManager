...
public class LanguageController extends AbstractController {
	...
	public List<SelectItem> getAvailableLanguages() {
		...
		for (Locale locale : languages) {
			selectItemsList.add(new SelectItem(locale.getLanguage(), locale.getDisplayLanguage(language)));
		}
	...
	private Locale getLocaleForCurrentUser() {
		...
		if(username != null) {
			String language = settingsService.getApplicationSettingValue(ApplicationSettingsService.GENERAL_LANGUAGE, username);
			if(language != null) {
